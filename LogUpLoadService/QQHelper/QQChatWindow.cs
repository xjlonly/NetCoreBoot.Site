...
namespace LogUpLoadService.QQHelper
{
    ...
    /// QQ聊天窗口类
    /// </summary>
    public class QQChatWindow : IDisposable
    {
        Log5 log = new Log5();
        ...
        public virtual void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                ...
                if (_QQWindowHandle != IntPtr.Zero)
                {
                    _QQWindowHandle = IntPtr.Zero;
                    Marshal.FreeCoTaskMem(_QQWindowHandle);
                }
                disposedValue = true;
            }
        }
