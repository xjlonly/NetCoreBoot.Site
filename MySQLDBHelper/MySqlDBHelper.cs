...
namespace MySQLDBHelper
{
    ...
    {
        public string _connecton = "";
        public  DapperMySQLHelper()
        {
          
            _connecton = "User ID = coc_r; Password = 123456; Host = 139.199.117.109; Database = coc; Pooling = true;";
        }
        ...
        {
            int result = 0;
            using (MySql.Data.MySqlClient.MySqlConnection con = new MySqlConnection(_connecton))
            {
                if (flag)
                {
        ...
        {
            int result = 0;
            using (MySqlConnection con = new MySqlConnection(_connecton))
            {
                if (flag)
                {
        ...
        /// <param name="flag">true存储过程，false sql语句</param>
        /// <returns>object</returns>
        public async Task<object> ExecuteScalarAsync<T>(string cmd, DynamicParameters param, bool flag = true) where T : class, new()
        {
            object result = null;
            using (MySqlConnection con = new MySqlConnection(_connecton))
            {
